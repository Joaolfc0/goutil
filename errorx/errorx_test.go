...
func TestTo_ErrorX(t *testing.T) {
	...
	assert.Contains(t, ex.Location(), "github.com/Joaolfc0/goutil/errorx_test.returnXErr(), errorx_test.go")
	assert.Eq(t, "an error message", ex.Message())
	assert.Contains(t, ex.StackString(), "github.com/Joaolfc0/goutil/errorx_test.returnXErr()")

	fn := ex.CallerFunc()
	assert.NotNil(t, fn)
	assert.Eq(t, "github.com/Joaolfc0/goutil/errorx_test.returnXErr", fn.Name())
